    ...
    function initTiltEffect(selector, options = {}) {
        ...
        document.querySelectorAll(selector).forEach((card) => {
            card.addEventListener("mousemove", (e) => {
                ...
                const px = (e.clientX - bounds.left) / bounds.width;  // 0 -> 1 across the card
                const py = (e.clientY - bounds.top) / bounds.height;  // 0 -> 1 down the card

                // Center-relative offset (-0.5 -> 0.5), inverted on the X axis
                // because rotating around the X axis tilts the *top* toward you
                // when the pointer is near the bottom, and vice versa.
